window.renderCertAdd = function(certIdOrChallengeId) {
  ...
  if (goalSelect) {
    ...
    const goalIdMap = challenge?.goalIdMap || {};
    if (goals.length === 0) {
      ...
      goals.forEach(goalContent => {
        const option = document.createElement('option');
        option.value = goalIdMap[goalContent]; // ✅ id로 매핑
        option.textContent = goalContent;

        if (editMode && editingCert && editingCert.goal === goalContent) {
  ...
  // 6. 이미지 미리보기 처리 (초기 로드 & 업로드 시)
  if (imgInput && imgPreview) {
    const previewImgTag = imgPreview.querySelector("img.real-preview");
    const previewIcon = imgPreview.querySelector(".cert-add-img-upload-icon");

    // 6-1. 수정모드일 때 기존 이미지 미리보기
    if (editMode && editingCert && editingCert.imgDataUrl) {
      if (previewImgTag) {
        previewImgTag.src = editingCert.imgDataUrl;
        previewImgTag.style.display = 'block';
      }
      if (previewIcon) {
        previewIcon.style.display = 'none';
      }
      imgPreview.classList.add("uploaded");
      imgDataUrl = editingCert.imgDataUrl;
    }

    // 6-2. 이미지 업로드 시 미리보기 갱신
    imgInput.onchange = function (e) {
      const file = e.target.files[0];
      if (file && previewImgTag) {
        const reader = new FileReader();
        reader.onload = function (ev) {
          imgDataUrl = ev.target.result;

          previewImgTag.src = imgDataUrl;
          previewImgTag.style.display = 'block';
